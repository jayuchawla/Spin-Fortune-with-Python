...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        entered_val = input("{} has ${}\n\nCategory: {}\nPhrase: {}\nGuessed: {}\n\nGuess a letter, phrase, or type 'exit' or 'pass':".format(self.name,self.prizeMoney,category,phrase,guessed))
        return entered_val

# Write the WOFComputerPlayer class definition (part C) here
class WOFComputerPlayer(WOFPlayer):
    SORTED_FREQUENCIES="ZQXJKVBPYGFWMUCLDRHSNIOATE" #E has highest frequency
    lis = [letter for letter in SORTED_FREQUENCIES]
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if len(letters)==0:
            return 'pass'
        smart_move = self.smartCoinFlip()
        if smart_move:
            for x in range(25,-1,-1):
                if self.SORTED_FREQUENCIES[x] not in guessed:
                    if self.SORTED_FREQUENCIES[x] in VOWELS:
                        if self.prizeMoney > 250:
                            return self.SORTED_FREQUENCIES[x]
                    else:
                        return self.SORTED_FREQUENCIES[x]
        else:
